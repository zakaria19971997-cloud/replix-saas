...
<section class="relative min-h-screen overflow-hidden bg-[#f7fbf8] px-4 py-10">
    ...
    <div class="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-[620px] flex-col items-center justify-center">
        ...
        <div class="w-full rounded-[2rem] border border-white/70 bg-white/95 p-10 pb-14 shadow-[0_30px_80px_rgba(27,67,45,0.10)] backdrop-blur md:p-12 md:pb-16">
            <div class="text-center" style="margin-bottom:48px;">
                ...
                <h1 class="font-bold tracking-tight text-[#16231b]" style="margin-top:0;margin-bottom:16px;font-size:36px;line-height:1.1;">{{ __("Create your account") }}</h1>
                <p class="text-base leading-8 text-[#698174]" style="margin-top:0;">
                    {{ __("Create your account and launch a cleaner WhatsApp workflow for marketing and support.") }}
                </p>
            </div>

            <form class="actionForm" action="{{ module_url('do_signup') }}" method="POST">
                <div class="space-y-5">
                    <div>
                        <label for="fullname" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Full Name") }}</label>
                        <input type="text" id="fullname" name="fullname" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Enter your full name') }}" required>
                    </div>
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                            <label for="username" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Username") }}</label>
                            <input type="text" id="username" name="username" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Choose a username') }}" required>
                        </div>
                    </div>

                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            <label for="password" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Password") }}</label>
                            <input type="password" id="password" name="password" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Enter your password') }}" required>
                        </div>
                        <div>
                            <label for="password_confirmation" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Confirm Password") }}</label>
                            <input type="password" id="password_confirmation" name="password_confirmation" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Confirm your password') }}" required>
                        </div>
                    </div>

                    <div>
                        <label for="timezone" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Timezone") }}</label>
                        <select id="timezone" name="timezone" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" required>
                    ...
                    <label for="accep_terms" class="flex items-start gap-3 text-sm text-[#5f7568]">
                        <input class="mt-1 h-4 w-4 rounded border-[#cfe0d4] text-[#1f7a45] focus:ring-[#1f7a45]" id="accep_terms" name="accep_terms" type="checkbox" value="1" required>
                        <span>
                            {{ __("I agree to the") }}
                ...
                <div style="padding-top:32px;">
                    ...
                    <button type="submit" style="display:flex;width:100%;height:56px;align-items:center;justify-content:center;border:none;border-radius:18px;background:#1f7a45;color:#ffffff;font-size:16px;font-weight:600;box-shadow:0 18px 40px rgba(31,122,69,0.22);cursor:pointer;">
                        <span style="color:#ffffff;">{{ __('Sign Up') }}</span>
                    </button>
                </div>
            </form>
            ...
            <p class="text-center text-sm text-[#66796d]" style="margin-top:28px;">
                ...
                <a href="{{ url('auth/login') }}" class="font-semibold text-[#1f7a45] hover:text-[#176338]">{{ __("Sign in") }}</a>
            </p>
        </div>
