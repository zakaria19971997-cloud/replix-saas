@php $captcha = Captcha::render(); @endphp
    ...
    <div class="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-[560px] flex-col items-center justify-center">
        ...
        <div class="w-full rounded-[2rem] border border-white/70 bg-white/95 p-10 pb-14 shadow-[0_30px_80px_rgba(27,67,45,0.10)] backdrop-blur md:p-12 md:pb-16">
            <div class="mb-10 text-center">
                ...
                <h1 class="font-bold tracking-tight text-[#16231b]" style="margin-top:0;margin-bottom:16px;font-size:36px;line-height:1.1;">{{ __("Create a new password") }}</h1>
                <p class="mt-4 text-base leading-8 text-[#698174]">
                    {{ __("Choose a strong password to secure your account and return to your workspace.") }}
                </p>
            </div>

            <form class="actionForm" action="{{ module_url('do_recovery_password') }}" method="POST">
                <input type="hidden" name="token" value="{{ $token ?? request('token') }}">
                <input type="hidden" name="email" value="{{ $email ?? request('email') }}">

                <div class="space-y-5">
                    <div>
                        <label for="password" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("New Password") }}</label>
                        <input id="password" type="password" name="password" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Enter new password') }}" required autocomplete="new-password">
                    </div>

                    <div>
                        <label for="password_confirmation" class="mb-2 block text-sm font-semibold text-[#2e4337]">{{ __("Confirm New Password") }}</label>
                        <input id="password_confirmation" type="password" name="password_confirmation" class="h-14 w-full rounded-[1.15rem] border border-[#dcebe0] bg-[#fbfdfb] px-4 text-[#16231b] outline-none transition focus:border-[#1f7a45] focus:bg-white" placeholder="{{ __('Confirm new password') }}" required autocomplete="new-password">
                    </div>
                ...
                <div class="pt-8">
                    ...
                    <button type="submit" style="display:flex;width:100%;height:56px;align-items:center;justify-content:center;border:none;border-radius:18px;background:#1f7a45;color:#ffffff;font-size:16px;font-weight:600;box-shadow:0 18px 40px rgba(31,122,69,0.22);cursor:pointer;">
                        <span style="color:#ffffff;">{{ __('Reset Password') }}</span>
                    </button>
                </div>
            </form>
